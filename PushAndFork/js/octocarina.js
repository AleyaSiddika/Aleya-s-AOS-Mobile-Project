  ...
  if(touchable){ touchable.addEventListener("touchstart", function(e) 
   {
      ...
      var dy= e.changedTouches[0].pageY;
       ...
       if(dx>9 && dx<30 && dy>267 && dy<293){
    	   move_player(-1, 0);  //left
        }else if(dx>28 && dx<52 && dy>250 && dy<269){
        	move_player( 0,-1);   //up
        }else if(dx>50 && dx<71 && dy>267 && dy<293){
        	move_player( 1, 0); //right
        }else if(dx>28 && dx<52 && dy>291 && dy<309){
        	move_player( 0, 1);  //down
        }else if(dx>423 && dx<453 && dy>279 && dy<305){
        	try_again(); //reload
        }else if(dx>467 && dx<496 && dy>280 && dy<306){
        	player_uses_fork();  //Z
        }
